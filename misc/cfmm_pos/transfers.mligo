...
[@inline] let const_x_token_entrypoint = "KT1..."  (* CHANGEME *)
[@inline] let const_y_token_entrypoint = "KT1..."  (* CHANGEME *)

(* Helper functions to make transfers in asset x and y. *)
let x_transfer (from : address) (to_ : address) (amount : nat) : operation =
...
#if X_IS_FA2
    Tezos.transaction [(from, [(to_, (const_x_token_id, amount))])] 0mutez x_contract
#else
    Tezos.transaction (from, (to_, token_amount)) 0mutez x_contract
#endif

let y_transfer (from : address) (to_ : address) (amount : nat) : operation =
    ...
    | Some contract -> contract in
#if Y_IS_FA2
    Tezos.transaction [(from, [(to_, (const_y_token_id, amount))])] 0mutez y_contract
#else
    Tezos.transaction (from, (to_, token_amount)) 0mutez y_contract
#endif
